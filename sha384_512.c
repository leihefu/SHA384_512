...
void sha384_512_init(sha384_512_ctx_t* ctx, int32_t is_384)
{
    SHA512_VALIDATE(ctx != NULL);

    memset(ctx, 0, sizeof(sha384_512_ctx_t));
...
static int32_t sha384_512_process(sha384_512_ctx_t* ctx, const uint8_t data[128])
{
    int32_t i;
 ...
 */
void sha384_512_update(sha384_512_ctx_t* ctx, const uint8_t* data, uint32_t len)
{
    int32_t ret;
    uint32_t fill;
    uint32_t left;
    ...
    if (left && len >= fill) 
    {
        ...
        if ((ret = sha384_512_process(ctx, ctx->buffer)) != 0) 
        {
            /* error */
            return;
        }
    ...
    while (len >= 128) 
    {
        if ((ret = sha384_512_process(ctx, data)) != 0) 
        {
            /* error */
            return;
        }
 ...
 */
void sha384_512_final(sha384_512_ctx_t* ctx, uint8_t* digest)
{
    int32_t ret;
    unsigned used;
    uint64_t high, low;
...
void sha384(const uint8_t* message, uint32_t len, uint8_t* digest)
{
    ...
    sha384_512_init(&ctx, 1);
    sha384_512_update(&ctx, message, len);
    sha384_512_final(&ctx, digest);
}
